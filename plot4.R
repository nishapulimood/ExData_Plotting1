...
## Set the plotting area to accept 4 independent plots, then input  the set of commands
## for each different plot in sequence.
par(mfcol=c(2,2))

#top-left plot
with(EPCsubset, plot(Time, Global_active_power, 
                     type="n", ylab="Global Active Power", xlab=""))
with(EPCsubset, lines(Time, Global_active_power))
...
# top-right plot
with(EPCsubset, plot(Time, Voltage, 
                     type="n", ylab="Voltage", xlab="datetime"))
with(EPCsubset, lines(Time, Voltage))

# bottom-right plot
with(EPCsubset, plot(Time, Global_reactive_power, type="n", xlab="datetime"))
with(EPCsubset, lines(Time, Global_reactive_power))

## Saving the screen device as a PNG plot
